...
export class AirportService {
    ...
    try {
      ...
      if (!name || name.trim() === '') {
        throw new BadRequestException(
          'El nombre del aeropuerto no puede estar vacío',
        );
      }
      ...
      throw new InternalServerErrorException(
        'Error al buscar aeropuerto por nombre',
      );
    }
  }
  ...
  public async getAirportByName(name: string): Promise<Airport | null> {
    try {
      ...
      // Error de base de datos o conexión
      throw new Error(
        `DATABASE_ERROR: Error al buscar aeropuerto por nombre: ${error instanceof Error ? error.message : 'Error desconocido'}`,
      );
    }
  }
  ...
  public async getAirportByIATA(IATA: string): Promise<Airport | null> {
    try {
      ...
      if (!IATA || IATA.trim() === '') {
        throw new BadRequestException(
          'El IATA del aeropuerto no puede estar vacío',
        );
      }
      ...
      if (!airport) {
        throw new NotFoundException(
          'No se encontró ningún aeropuerto con ese IATA',
        );
      }
