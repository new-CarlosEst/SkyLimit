...
import { FlightEntity, LegEntity, StopoverEntity, AirportDetail } from './entities/flight.entity';
...
export class ApiflightService {
    ...
    public async getFlightsRoundTrip(
        ...
        const originEntityId = await this.getAirportIdByName(originName, originIATA);
        const destinationEntityId = await this.getAirportIdByName(destinationName, destinationIATA);

        //Verifico que la  clase del vuelo es valida
        let cabinClassVerified = this.verifyCabinClass(cabinClass);
    ...
    private async getAirportIdByName(name: string, IATA: string) {
        ...
        try {
            ...
            const airport = response.data.places.find((airport: any) => airport.skyId === IATA);
    ...
    private async mapToFlightEntity(apiResponse: any): Promise<FlightEntity[]> {
        ...
        const mappedItineraries = await Promise.all(itineraries.map(async (itinerary: any) => {
            const legs = await Promise.all(itinerary.legs.map(async (leg: any) => {
                // Obtengo los datos del aeropuerto de origen y destino por su IATA consultando mi propia base de datos
                const originDetail = await this.airportService.getAirportByIATA(leg.origin);
                const destinationDetail = await this.airportService.getAirportByIATA(leg.destination);
